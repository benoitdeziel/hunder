...
const ProductForm = ({ product }) => {
  ...
  const checkAvailability = productId => {
    context.client.product.fetch(productId).then(product => {
      ...
      const result = product.variants.filter(variant => variant.id === productVariant.shopifyId);
  ...
  const variantSelectors = hasVariants
    ? product.options.map(option => (
        <VariantSelector onChange={handleOptionChange} key={option.id.toString()} option={option} />
      ))
    : null;
